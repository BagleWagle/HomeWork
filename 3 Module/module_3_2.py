def send_email(message, recipient, sender='university.help@gmail.com'):
    if not recipient.endswith('.ru') and not recipient.endswith('.net') and not recipient.endswith('.com') and sender != 'university.help@gmail.com' or recipient.find('@') < 0:
        print(f'Невозможно отправить письмо с адреса {sender} на адрес {recipient}, так еще и НЕСТАНДАРНЫЙ ОТПРАВИТЕЛЬ!')
    else:
        if not recipient.endswith('.ru') and not recipient.endswith('.net') and not recipient.endswith('.com') or recipient.find('@') < 0:
